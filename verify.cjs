...
E.length=0;p.charge=10;bubblePower();if(orbs.length!==24)throw Error('Empty bubbles missing');orbs.splice(0,orbs.length,{x:p.x+15,y:p.y+p.h+24,vx:0,vy:0,life:6,burst:1});p.vy=100;updateOrbs(.001);if(p.vy!==-620||p.jump!==2||orbs.length)throw Error('Empty bubble bounce');reset();
...
J.t=1;update(.016);if(E.length!==1||!E[0].boss||p.hp!==8||p.charge!==10||zone!==4)throw Error('Boss teleport');let testBoss=E[0];J.t=1;update(.016);if(E[0]===testBoss||E[0].hp!==60)throw Error('Boss teleport restart');reset();
...
weapon=3;p.energy=0;E.length=0;let blastTarget={x:p.x+100,y:p.y,hp:100};E.push(blastTarget);
...
if(p.reload!==1||attack!==1)throw Error('One second blast cooldown');if(p.energy!==0)throw Error('Blast consumes energy');reset();
...
reset();E.length=0;p.energy=.1;K.z=input==='key';mouse.down=input==='mouse';
...
reset();if(p.lc!==0)throw Error('Restart timers failed');
